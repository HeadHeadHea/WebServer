...
public:
    /* 初始化新接受的连接 */
    void init(int sockfd, const sockaddr_in &addr, const char* root = "/root/Project/WebServer/root");
    /* 关闭连接 */
    ...
    /* 设置数据库连接*/
    void setSqlCon(std::shared_ptr<SqlCon> sql_con) { this->sql_con = sql_con; }
    /* 获取文件描述符*/
    int getFd() {return m_sockfd;}
    /* 打印读缓冲区的内容*/
    char* printRead(){return m_readBuf;}
    /* 打印写缓冲区的内容*/
    char* printWrite(){return m_writeBuf;}
    /* 打印待写入字节数*/
    void printBytes(){printf("m_writeIdx = %d\n", m_writeIdx);}
...
private:
    ...
    bool getNameAndPwd(std::string &name, std::string &password);
    /* 处理不同CGI的方法 */
    void CGI_UserLog();
    void CGI_UserRegist();
...
extern std::unordered_map<std::string, std::string> m_users;
